...
<head>
    ...
    <!-- Inclusion dinamica de recursos de los paquetes activos -->
    @foreach(File::directories(base_path('vendor/dvzambrano')) as $packageDir)
        @php
            $packageName = basename($packageDir);
            $includePath = $packageDir . '/Resources/views/include';
        @endphp
        ...
        @if(File::isDirectory($includePath))
            ...
            @foreach($files as $file)
                @php
                    ...
                    $fullViewName = "{$packageName}::include.{$viewPartial}";
                @endphp
